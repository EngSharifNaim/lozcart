...
class MessagesController extends Controller
{
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request['dark_mode']) {
            $request['dark_mode'] == "dark"
                ? Market::where('id', Auth::user()->id)->update(['dark_mode' => 1])  // Make Dark
                : Market::where('id', Auth::user()->id)->update(['dark_mode' => 0]); // Make Light
        }
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    $update = Market::where('id', Auth::user()->id)->update(['avatar' => $avatar]);
    ...
    public function setActiveStatus(Request $request)
    {
        $update = $request['status'] > 0
            ? Market::where('id', $request['user_id'])->update(['active_status' => 1])
            : Market::where('id', $request['user_id'])->update(['active_status' => 0]);
        // send the response
        return Response::json([
            'status' => $update,
        ], 200);
