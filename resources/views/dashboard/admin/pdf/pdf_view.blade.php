        ...
        <tr>
            <td align="center" style="width: 50%;vertical-align:top">
                <img width="64" height="100" style="border-radius: 8px;" src="data:image/jpeg;base64,
                    {{ base64_encode(@file_get_contents(url(env('BACKEND_URL').$medical_center->logo))) }}">
                    <br>
                   Clinic Name : {{$medical_center->name}}<br>
                   Address : {{$medical_center->address ??'jh'}}<br>
                    Email : {{$medical_center->email}}<br>
                    Mobile :{{$medical_center->mobile}}
            </td>
        </tr>
...
<table class="table-fill">
    ...
    <tr>
        ...
        <td>{{$visit->appointment_time ??__('Not Found')}}</td>
        <td>{{$visit->cost_appointment}}</td>
        <td>{{$visit->patient_insurance->program->name}}</td>

    </tr>
        <tr>
            <td colspan="5" class="text-left">Consultation :<br> {{$visit->consultation->doctor_notes}}</td>
        </tr>
    <tr>
        <td colspan="5" class="text-left">Prescription :<br> {{$visit->consultation->prescription->prescription_details}}</td>
        </tr>
    @endforeach
    </tbody>
</table>
