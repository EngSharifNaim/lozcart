...
@section('content')
                                ...
                                <div class="card-body">
                                    ...
                                    <ul class="orderStatus">
                                        ...
                                        <li title="{{__("Status Update")}}" class="mr-1 mb-2 {{$order->status ==3?'active':''}}">
                                            <a href="#" data-id="{{$order->id}}" data-status="3" data-next="completed" id="btn_delivering" class="btn btn-success btn-rounded sa-params ">
                                                {{__("Delivering")}}
                                            </a>
                                        </li>
                                ...
                                <div class="card-body">
                                    <table class="table table-bordered table-striped">
                                                ...
                                                @if ($order->use_coupon)
                                                    ...
                                                    @endif
                                                        @if ($order->use_coupon->type ==1)
                                                            {{$order->use_coupon->coupon_value *100}}%
                                                        @endif
                                                @endif
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td>
                                                <span><b style="color: #096d3e;">{{$order->total_price +$order->shipping->price}} {{$order->currency->currency_short_ar}} </b></span>
                                            </td>
                                        </tr>
                                                                            ...
                                                                            <div style="margin-top: 15px;">
                                                                                ...
                                                                                <table style="width:570px;max-width:570px;margin:0 auto;padding:0;margin-bottom:20px;padding-bottom:10px;border-collapse: collapse;width: 100% !important;border:1px solid #edeff2;" align="center" cellpadding="0" cellspacing="0" class="produtctTableDetails">
                                                                                    ...
                                                                                    <tr>
                                                                                        ...
                                                                                        <td align="{{App::isLocale('en')?'right':'left'}}" style="padding: 10px">
                                                                                            {{$order->total_price +$order->shipping->price}} {{$order->currency->currency_short_ar}}
                                                                                        </td>
                                                                                    </tr>
                                                                                    </tbody>
                                                                                </table>
    ...
    <div id="transactionModal" class="modal fade me-custom-modal show" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-modal="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header"  style="    background-color: #096d3e00;">
                    <h4 class="modal-title" style="    opacity: 0">{{__('Transaction')}}</h4>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-12">
                                <img class="w-100" src="{{env('ATTACH_URL_LOZ'). $order->payment_attach}}" alt="">
                            </div>
                        </div>
                    </div>
    ...
    @endif
    <div id="btn_delivering_modal" class="modal fade me-custom-modal show" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-modal="true">
        <div class="modal-dialog">
            <div class="modal-content">


                <form method="POST" enctype="multipart/form-data" id="confirm_delivering_form" novalidate="novalidate">
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-row">
                                    ...
                                    <div class="form-group col-md-9">
                                        <input required=""  id="shipping_name" type="text" name="shipping_name" class="form-control" placeholder="{{__('Shipping company name')}}" aria-required="true">
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-group col-md-9">
                                        <input required=""  id="tracking_number" type="text" name="tracking_number" class="form-control" placeholder="{{__('Tracking number')}}" aria-required="true">
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-9 offset-md-3">
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" name="need_tracking_no" id="need_tracking_no" class="custom-control-input"  >
                                            <label class="custom-control-label" for="need_tracking_no">{{__('This order does not need a tracking number.')}}</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" data-status="3" data-next="completed" id="confirm_delivering" class="btn btn-success waves-effect waves-light">{{__('Confirm !')}}</button>
...
{{--    change status--}}
        ...
        $(document).ready(function () {
            $(document).on('click', '.statusBtn', function (e) {
                ...
                if (status != current_status) {
                    swal.fire({
                            title: "{{__('Confirm the order status change process!')}}",
                            type: "warning",
    ...
    {{--    modal btn_delivering --}}
    <script>
        $(document).ready(function () {
            $(document).on('click', '#btn_delivering', function (e) {
                $('#btn_delivering_modal').modal('show');
            });
            });
    </script>
    <script>
        $('#need_tracking_no').on("change",function ()
        {
            if ($('#need_tracking_no').is(":checked")){
    ...
    <script >
        $(document).on("click", "#confirm_delivering", function() {
            ...
            var btn = $('#btn_delivering');
            var id = {{$order->id}};
            var status = $('#btn_delivering').data('status');
            console.log(btn.text())
            var next = $('#btn_delivering').data('next');
            var current_status = "{{$order->status}}";
            console.log('status'+status +'c-status'+current_status)
            var $form = $(this.form);
            if(! $form.valid()) {
            ...
            if ($form.valid()) {
                $.ajax({
                    type: "POST",
                    url: "{{route('order.confirm_delivering',$order->id)}}",
                    data: postData,
                    processData: false,
                    contentType: false,
                    beforeSend: function () {
                        $('#btn_delivering_modal').modal('hide');
                        $('.loader-modal').fadeIn(200);
                    },
                    success: function (data) {
                        if (data.status === 0) {
